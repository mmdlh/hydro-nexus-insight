...
function chartOption(kind: ChartKind, variant = 0): echarts.EChartsOption {
  ...
  if (kind === "scatter") return { ...base, grid: { top: 24, right: 18, bottom: 28, left: 42 }, xAxis: { ...axis, name: "运行时长", nameTextStyle: { color: "rgba(188,239,250,.4)" } }, yAxis: { ...axis, name: "健康度", nameTextStyle: { color: "rgba(188,239,250,.4)" } }, series: [{ type: "scatter", symbolSize: (value: number[]) => Math.max(8, value[2] / 3), data: [[12, 96, 28], [26, 88, 44], [33, 74, 36], [46, 65, 52], [58, 91, 24], [72, 48, 42], [81, 82, 33]] }] };
  const seriesType = kind === "bar" ? "bar" : "line";
  return { ...base, legend: { top: 4, left: "center", textStyle: { color: "rgba(188,239,250,.62)", fontSize: 10 } }, grid: { top: 40, right: 16, bottom: 24, left: 42 }, xAxis: { type: "category", data: labels, ...axis }, yAxis: { type: "value", ...axis }, series: [{ name: variant % 2 ? "实际值" : "供水量", type: seriesType, smooth: true, barMaxWidth: 22, data: [42, 58, 51, 76, 69, 88], lineStyle: { width: 3 }, areaStyle: kind === "area" || kind === "line" ? { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: "rgba(32,184,242,.42)" }, { offset: 1, color: "rgba(32,184,242,0)" }]) } : undefined }, ...(variant > 1 ? [{ name: "目标值", type: "line" as const, smooth: true, data: [45, 54, 60, 72, 74, 82], lineStyle: { color: "#39E6A7", type: "dashed" as const } }] : [])] };
}
